...
Historical planning documents from Book Vault development (December 2025 – August 2026).
...
| **Dependency Majors (Jul–Aug 2026)**  | The full upgrade ladder: Next 14→16, React 18→19, Prisma 5→6→7 (+ ESM), Storybook/redocly/openapi-typescript, Node 20→24, cross-arch amd64 → native arm64/Graviton, and the August audit cleanup (#88–#101, #150–#151) |
...
- `dependency-major-upgrades.md` — the phased major-dependency ladder (✅ all phases). Worth
  re-reading before the next framework major: each phase records what actually broke, including
  the Next 16 qemu/SWC segfault that forced the arm64 move, Prisma 7's driver-adapter + gitignored-
  client fallout, and the CI rule that came out of it (`db:generate` must run right after
  `npm ci`). Still-open deferrals live in [plans/dependency-deferrals.md](../../plans/dependency-deferrals.md).
- `ios-swift6-generated-networking-plan.md` — stopped generating the unused OpenAPI request layer (✅ #142). Its Phase 3 was step A4 of `ios-modernization-sequencing.md`, which has since shipped too.
- `ios-modernization-sequencing.md` — the Plan A/B/C sequencing decision and its implementation notes (✅ A0–A6, #133–#136 + #144). Records why the Swift 6 error count is untrustworthy from a single build, and the Xcode-version differences that only CI could catch.
