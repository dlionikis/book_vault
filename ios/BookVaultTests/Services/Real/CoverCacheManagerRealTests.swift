...
@MainActor
final class CoverCacheManagerRealTests: XCTestCase {
    ...
    func testTokenAttachedForApiHostURL() async throws {
        ...
        XCTAssertEqual(capturedAuth, "Bearer \(token)",
    ...
    func testTokenNotAttachedForSameHostDifferentPort() async throws {
        ...
        XCTAssertNil(capturedAuth, "Port is part of the host guard; a different port must not get the token")
    }
    ...
    func testNoTokenWhenProviderReturnsNil() async throws {
        ...
        XCTAssertNil(capturedAuth, "No token available → no Authorization header even on the API host")
    }
    ...
    func testAlreadyCachedIssuesNoNetworkRequest() async throws {
        ...
        try await manager.cacheCover(for: bookId, from: coverURL)
        XCTAssertEqual(requestCount, 1, "Already-cached cover must not issue a second network request")
    }
